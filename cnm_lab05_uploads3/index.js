...
app.post('/save', upload.single('fileimage'), (req, res) => {
  ...
  s3.upload(paramsS3, async (err, data) => {
    if (err) {
      ...
      return res.status(500).send("Internal Server Error");
    } else { // khi upload hình ảnh lên S3 thành công
...
//   }
//   console.log(idcheck)
//   const deleteRequests = idcheck.map(id => ({
//     DeleteRequest: {
//       Key: {
//         id: id,
//       },
...
//   // if (!listcheckbox || listcheckbox.length <= 0) { // nếu ko có gì thì xóa
//   //   return res.redirect('/');
//   // }
//   // try {
//   //   function onDeleteItem(id) { // định nghĩa hàm đệ quy xóa item
//   //     const params = {
//   //       TableName: tableName,
//   //       Key: {
//   //         id: listcheckbox[id],
...
//   //       } else if (length > 0) {
//   //         onDeleteItem(length - 1); // nếu vị trí cần xóa vẫn >0 thì gọi đệ quy xóa tiế[]
//   //       } else {
//   //         return res.redirect('/'); // reload lại trang index
//   //       }
//   //      }
//   //   }
  //   onDeleteItem(listcheckbox.length - 1); // gọi hàm đệ quy xóa item
  // } catch (error) {
